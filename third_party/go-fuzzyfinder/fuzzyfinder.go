...
func (f *finder) _draw() {
	...
	// Clear cells in-memory instead of calling f.term.Clear()
	// which triggers slow Win32 FillConsoleOutputCharacter on Windows,
	// causing visible screen flickering in Windows Terminal.
	for y := 0; y < height; y++ {
		for x := 0; x < width; x++ {
			f.term.SetContent(x, y, ' ', nil, tcell.StyleDefault)
		}
	}
